...
class GaussianModel:
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii, viewpoint_cam: Camera, iteration):
        ...
        if max_screen_size:
            ...
            if iteration % 100 == 0:
                # print max_radii2D
                print(f"[Densification] Max Radii2D: {self.max_radii2D.max()}, max_screen_size: {max_screen_size}")
                print(f"[Densification] {torch.sum(big_points_vs)} points have a max radius2D larger than {max_screen_size}")
                print(f"[Densification] {torch.sum(big_points_ws)} points have a max scaling larger than 0.05 * extent: ({0.05 * extent})")
            prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)

        if outside_obj_mask is not None:
            prune_mask = torch.logical_or(prune_mask, outside_obj_mask)
